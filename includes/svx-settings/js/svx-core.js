...
(function($){
	...
	function __add_events() {
		...
		$(document).on( 'svx-fields-on-screen', function(e) {
			...
			if ( $('#svx-duplicate-filter').length==0 && $('#_filter_preset_manager').length>0 ) {
				$('#_filter_preset_manager').after('<span id="svx-duplicate-filter" class="svx-button">Duplicate</span><span id="svx-new-filter" class="svx-button">New</span><span id="svx-delete-filter" class="svx-button">Delete</span>');
			}
	...
	function __start_up() {
		svx.ajax_factory = {};
		svx.language = u($('#svx-main-wrapper').attr('data-language'))?$('#svx-main-wrapper').attr('data-language').toString().substring(1):false;
	...
	function __get_filter_name(c) {
		var p = prompt( 'Preset name', c=='new'?'New':$.trim($('#_filter_preset_manager option:selected').text())+' Copy' );
	...
	function _set_term_name(e) {
		return e.length>15?e.substring(0, 13)+'..':e;
	}

	function set_term_name(e) {
		var $e = $(e.target);
		$e.closest('.svx-terms-list-item').find('.svx-term-item-title:first').html($e.val().length>15?$e.val().substring(0, 13)+'..':$e.val());
	}
