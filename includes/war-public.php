...
?>
		...
		<?php printf( '<cite class="fn">%s</cite> <span class="says">' . esc_html__( 'says', 'woocommerce-warranties-and-returns' ) . ':</span>', get_comment_author_link() ); ?>
		</div>
		<div class="war_comment_body war_comment_<?php comment_ID() ?>">
		<?php endif; ?>
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em class="war_awaiting_moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'woocommerce-warranties-and-returns' ); ?></em>
			<br />
		<?php endif; ?>

		<div class="war_comment_meta"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				printf( esc_html__('%1$s at %2$s'), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_html__( '(Edit)', 'woocommerce-warranties-and-returns' ), '  ', '' );
			?>
		</div>
